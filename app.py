...
def get_group_details(group_id):
    ...
    settlements = conn.execute(
        ...
        "WHERE t1.group_id = ? AND t1.type = 'SETTLEMENT' AND t1.amount > 0 "
        "AND t2.type = 'SETTLEMENT' AND t2.amount < 0 AND t1.id = t2.id - 1",
        (group_id,)
    ).fetchall()
...
def get_net_balances(group_id):
    conn = get_db()
    members = [row['name'] for row in conn.execute("SELECT name FROM members WHERE group_id = ?", (group_id,)).fetchall()]
    balances = {name: 0 for name in members}
    cursor = conn.execute("SELECT member_name, SUM(amount) as net FROM transactions WHERE group_id = ? GROUP BY member_name", (group_id,))
...
def calculate_simplified_debts(balances):
    ...
    while debtors and creditors:
        ...
        cred_bal_neg, creditor = heapq.heappop(creditors)
        cred_bal = -cred_bal_neg
...
def view_group(group_id):
    conn = get_db()
    groups = conn.execute("SELECT * FROM groups WHERE id = ?", (group_id,)).fetchall()
